...
/// BEGIN OLED
    ...
    //% blockId=NL_OLED_WriteLine 
    //% block="OLED show line %line|text %text"
    //% line.min=0 line.max=7 line.defl=1
    //% text.defl="goodbye, daisy"
    //% subcategory=Display group="Display"
    export function writeLine(line: number, text: string) {
        if (text.length > 16) {
    ...
    //% blockId=NL_OLED_WriteChar 
    //% block="OLED write char %char on row %row at column %col"
    //% line.min=0 line.max=7 line.defl=0
    //% col.min=0 col.max=15 col.defl="0"
    //% subcategory=Display group="Display"
    export function writeChar(char: string, row: number, column: number) {
        setText(row, column)
        // writeCustomChar(basicFont[char.charCodeAt(0) - 32]);
